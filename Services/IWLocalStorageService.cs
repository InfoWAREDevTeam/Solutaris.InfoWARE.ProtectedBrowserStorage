...
namespace Solutaris.InfoWARE.ProtectedBrowserStorage.Services
{
    internal class IWLocalStorageService : IWProtectedBrowserStorageBase, IIWLocalStorageService
    {
        ...
        public void SetItem<T>(string key, T value)
        {
            ...
            {
                IJSInProcessRuntime jSInProcess = (IJSInProcessRuntime)m_jSRuntime;
                jSInProcess.InvokeVoid("setItemToLocalStorage", key, JsonSerializer.Serialize(value), m_encryption.Key);
            }
            catch (Exception ex)
            {
                return;
        ...
        public async Task SetItemAsync<T>(string key, T value)
        {
            try
            {
                await m_jSRuntime.InvokeVoidAsync("setItemToLocalStorage", key, JsonSerializer.Serialize(value), m_encryption.Key);
